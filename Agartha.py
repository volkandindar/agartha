...
"""
    ...
    def funcBambdasRun(self, ev):
        ...
        for line in self._tbBambdasScopeDoneURLs.getText().splitlines():
            if line.strip().startswith("/*") or line.strip() == "/":
                self._lblBambdasNotification2.text = "You can not set everything done in the done URLs."
        ...
        for line in self._tbBambdasBlackListedURLs.getText().splitlines():
            if line.strip().startswith("/*") or line.strip() == "/":
                ...
                if self._tbBambdasScopeURLs.text == self._txBambdasScopeURLs or self._tbBambdasScopeURLs.text.strip() == "" or self._tbBambdasScopeURLs.text.strip().startswith("/*"):
                    self._lblBambdasNotification2.text = "You enter root directory (/) in blacklisted URLs, you need to provide scope URLs or remove root directory (/) from the scope URL list!"
                    self._lblBambdasNotification2.setForeground(Color.red)
                    return
        ...
        if self._tbBambdasScopeDoneURLs.text != self._txBambdasScopeDoneURLs and self._tbBambdasScopeDoneURLs.text.strip() != "":
            ...
            for line in self._tbBambdasScopeDoneURLs.text.splitlines():
                if  line.strip() == "":
                    ...
                    self._lblBambdasNotification2.text = "All URLs in done URLs should start with '/'"
                    self._lblBambdasNotification2.setForeground(Color.red)
                    return
                else:
                    targetPaths += "\"" + (line.strip().replace("*",".*") + ".*").replace(".*.*", ".*") + "\", "
...
"""
        ...
        if allUrls:
            ...
            if allUrlsBlacklisted:
                self._lblBambdasNotification2.text = "Script generated and copied to clipboard. '" + str(sum(1 for line in self._tbBambdasScopeURLs.text.splitlines() if line.strip().startswith('/'))) + "(Scope) + "+ str(sum(1 for line in self._tbBambdasScopeDoneURLs.text.splitlines() if line.strip().startswith('/'))) + "(Done)' endpoints will be highlighted, and the rest is blacklisted!"
            else:
                self._lblBambdasNotification2.text = "Script generated and copied to clipboard. Includes '" + str(sum(1 for line in self._tbBambdasScopeURLs.text.splitlines() if line.strip().startswith('/'))) + "(Scope) + " + str(sum(1 for line in self._tbBambdasScopeDoneURLs.text.splitlines() if line.strip().startswith('/'))) + "(Done)' endpoints and '" + str(sum(1 for line in self._tbBambdasBlackListedURLs.text.splitlines() if line.strip().startswith('/'))) + "' blacklisted URLs."
    ...
    def _tabBambdasUI(self):
        ...
        self._tbBambdasScopeDoneURLs.setToolTipText("Done URLs")
        self._txBambdasScopeDoneURLs = "Done URLs, same rules are also applicable for here!"
        ...
        self._lbBambdasScopeDoneURLs = JLabel("Done URLs", SwingConstants.LEFT)
        ...
        self._txBambdasBlackListedURLs = "Blacklist urls will be in here, same URL definition rules are applicable, for example: \n\t-/health-check\n\t+ Includes specifically this path, and rest, for example:\n\t\t+ /health-check\n\t\t+ /health-check/Monitor\n\t\t+ /health-check/?Level=Info\n\nNote1: Leave it as it is to add nothing to this list.\nNote2: If you want to show only items in the scope, you can put here /* to ignore everything except the scope URLs."
        ...
        self._lbBambdasBlackListedURLs = JLabel("Black-Listed / Unwanted URLs", SwingConstants.LEFT)
