...
"""
    ...
    def funcBambdasRun(self, ev):
        ...
        bambdas += " * Highlights and annotates Burp history to surface potential leads.\n"
        bambdas += " * Comments are concise and editable so you can adapt rules to your test.\n"
        bambdas += " **/\n\n"
        ...
        bambdas += "// Testing scope (allowlist / wanted URLs). Regex supported.\n"
        ...
        if self._tbBambdasBlackListedURLs.getText() != '/':
            bambdas += "// Blocklist (unwanted URLs). Regex supported.\n"
            if self._tbBambdasBlackListedURLs.text != self._txBambdasBlackListedURLs:
                ...
                bambdas += "// Add patterns here to ignore noise (health checks, static banners, etc.).\n"
            else:
                bambdas += "String[] targetBlackListUrls = {\"/YouCanPutBlackListURLsHere.*\"};\n"
                bambdas += "// Add patterns here to ignore noise (health checks, static banners, etc.).\n"
            bambdas += "\n"

        bambdas += "// Already-tested URLs (mark as completed). Regex supported.\n"
        ...
        if self._cbBambdasScope.isSelected():
            bambdas += "// Process only items that are in scope and have a response.\n"
            bambdas += "if (!requestResponse.hasResponse() || !requestResponse.request().isInScope())\n"
        else:
            bambdas += "// Process only items that have a response.\n"
            bambdas += "if (!requestResponse.hasResponse())\n"
        bambdas += "    return false;\n\n"

        if self._cbBambdasforWhat.getSelectedIndex() == 0:
            bambdas += "// Skip if already processed:\n"
            bambdas += "// - Non-default highlight (other than configured colors) suggests a different rule handled it.\n"
            bambdas += "// - If notes exist and don’t start with \"Suspicious\", preserve analyst notes.\n"
        ...
        if self._cbBambdasHTTPMethods.isSelected():
            ...
            bambdas += "// End HTTP methods to ignore\n\n"
        ...
        if self._cbBambdasValuable.isSelected() and (self._cbBambdasSearchinReq.isSelected() or self._cbBambdasSearchinRes.isSelected() or self._cbBambdasSearchinURL.isSelected()):
            ...
            bambdas += "// End high-value keywords\n\n"
        ...
        if self._cbBambdasSearchinRes.isSelected():
            if self._cbBambdasVulnJS.isSelected():
                ...
                vulnJSFunc += "}"
                bambdas += "// Potentially risky JavaScript functions (sinks) to look for in responses\n"
                bambdas += "String[] suspiciousFunctions = " + vulnJSFunc + ";\n"
                bambdas += "// End risky JavaScript function list\n\n"
            ...
            if self._cbBambdasFilesDownloadable.isSelected():
                ...
                fileExtensions += "}"
                bambdas += "// Potentially downloadable/sensitive file extensions to flag\n"
                bambdas += "String[] fileExtensions = " + fileExtensions + ";\n"
                bambdas += "// End file extension list\n\n"

        if self._tbBambdasBlackListedURLs.getText() != '/':
            bambdas += "// Apply blocklist early to skip unwanted URLs\n"
    ...
    """
        ...
        // Scan response for suspected downloadable/sensitive files
        for (Pattern pattern : patterns) {
            ...
            while (matcher.find()) {
                ...
                if (notesBuilder.length() > 0)
                    notesBuilder.append(", ");
                // NOTE: tag intentionally does not imply certainty
    ...
    """
            ...
            if self._cbBambdasSearchHTMLCommnets.isSelected():
                bambdas += """
        // HTML comments found in response (can reveal hidden toggles/todos)
        patterns = new ArrayList<>();
    ...
    """
            ...
            if self._cbBambdasSearchinReq.isSelected():
                ...
                // JSON or XML body indicators (pattern-based)
                for (Pattern pattern : patterns)
                    if (pattern.matcher(requestBody).find()){
                        suspiciousHit = true;
                        if (notesBuilder.length() > 0)
    ...
    """
        if self._tbBambdasBlackListedURLs.getText() == '/':
            bambdas += """
        // Root blocklist ("/") selected: ignore everything outside explicit scope/tested unless flagged as suspicious
        if (!suspiciousHit) {
